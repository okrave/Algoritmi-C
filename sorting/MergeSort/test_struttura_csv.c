...
void test_merge_sort_on_int_struct(){   
...
void test_merge_sort_on_string_struct(){
    record ** full_array_record = (record**) malloc (sizeof(record*)* NUM_MAX_RECORD);
    ...
    static clock_t start_time, end_time ;
    start_time = clock();
    msortBasic((void**)full_array_record,0,NUM_MAX_RECORD-1,compare_string_ptr_struttura);
    end_time = clock();
    printf("Array ordinato in %4.5f seconds\n\n", (end_time-start_time)/(double) CLOCKS_PER_SEC);
...
void test_merge_sort_on_double_struct(){ 
    record ** full_array_record = (record**) malloc (sizeof(record*)* NUM_MAX_RECORD);
    ...
    static clock_t start_time, end_time ;
    start_time = clock();
    msortBasic((void**)full_array_record,0,NUM_MAX_RECORD-1,compare_double_ptr_struttura);
    end_time = clock();
    printf("Array ordinato in %4.5f seconds\n\n", (end_time-start_time)/(double) CLOCKS_PER_SEC);
...
int main(){
    ...
    test(test_merge_sort_on_int_struct);
    end_tests();
    start_tests("Merge Sort \n\n **************STRING record with file csv************");
    test(test_merge_sort_on_string_struct);
    end_tests();
    start_tests("Merge Sort \n\n **************DOUBLE record with file csv************");
    test(test_merge_sort_on_double_struct);
    end_tests();
